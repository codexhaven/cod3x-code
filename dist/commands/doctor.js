    ...
    async execute() {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                const req = http.request({ hostname: 'localhost', port: 6446, path: '/v1/models', method: 'GET', timeout: 3000 }, (res) => resolve());
                req.on('error', () => reject());
                req.on('timeout', () => { req.destroy(); reject(); });
                req.end();
            });
            checks.push({ name: 'opencode-proxy', status: 'ok', message: 'Running on localhost:6446' });
        }
        catch {
