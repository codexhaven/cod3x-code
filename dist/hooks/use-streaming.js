...
export function useStreaming({ config, logger, llmFactory }) {
...
After receiving tool results, provide a final answer without tool tags.`;
            ...
            let fullResponse = '';
            const response = await llmFactory.stream(messages, (token) => {
                fullResponse += token;
            ...
            return `Error: ${error.message}. Check your proxy/API configuration.`;
        }
        finally {
