...
export class PlatformDetector {
    ...
    async detect() {
        ...
        const detectedType = isTermux ? 'termux'
            : isAndroid ? 'android'
    ...
    async checkTermux() {
        ...
        if (process.env.PREFIX?.includes('termux'))
            return true;
        if (process.env.HOME?.includes('termux'))
            return true;
