...
export const Cod3xApp = ({ config, logger, permissions, toolRegistry, fileIndex, memory, mcpManager, agentOrchestrator, llmFactory, platform, }) => {
    const { exit } = useApp();
    ...
    const handleSendMessage = useCallback(async (content) => {
        ...
        try {
            ...
            if (fileRefs) {
                ...
                const fileContents = await Promise.all(fileRefs.map(async (ref) => {
            ...
            if (toolCallMatch) {
                ...
                for (let turn = 0; turn < MAX_TURNS; turn++) {
                    ...
                    try {
                        ...
                        currentPrompt = response + '\n<tool_error>' + e.message + '</tool_error>\nTry again with correct parameters.';
                        addStep('error', `Tool execution error`, { error: e.message });
                    }
                }
                if (!finalResponse) {
    ...
    const handleSlashCommand = useCallback(async (command, args) => {
        switch (command) {
            case 'exit':
            case 'quit':
                exit();
            ...
            case 'tools':
                ...
                sendMessage(`Cod3x Tools (${tools.length}):\n${tools
                    .map((t) => `  [${t.category}] ${t.name} - ${t.description}`)
                    .join('\n')}`, 'system');
                break;
            case 'agents':
                const agents = agentOrchestrator.getAgents();
                sendMessage(`Cod3x Agents (${agents.length}):\n${agents
                    .map((a) => `  • ${a.name} (${a.role}) - ${a.description}`)
                    .join('\n')}\n\nSwarm: ${config.swarm.enabled ? 'enabled' : 'disabled'}`, 'system');
                break;
            case 'swarm':
                if (args.length > 0) {
                    setMode('swarm');
    ...
    return (_jsxs(Box, { flexDirection: "column", height: "100%", children: [_jsx(Header, { projectName: config.name, model: config.ai.model, provider: config.ai.provider, platform: platform.type, version: "4.0.0", mode: mode }), showBanner && (_jsx(Box, { justifyContent: "center", paddingY: 1, children: _jsx(Text, { color: "green", children: "Cod3x Code v4.0 by CodexHaven \u2014 Type /help for commands" }) })), _jsx(Box, { flexDirection: "column", flexGrow: 1, children: _jsx(ChatInterface, { messages: messages, onSendMessage: handleSendMessage, onSlashCommand: handleSlashCommand, isThinking: isThinking, isStreaming: isStreaming, config: config, trail: mode === 'trail' ? trail : undefined }) }), _jsx(StatusBar, { status: statusMessage, model: config.ai.model, tools: toolRegistry.list().length, agents: agentOrchestrator.getAgents().length, files: fileIndex.getStats?.().totalFiles || 0, tokens: memory.getTokenCount?.() || 0, platform: platform.type, mode: mode })] }));
};
