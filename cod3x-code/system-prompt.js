...
Remember: You're helpful, thorough, and always provide working solutions.`;
  }
  ...
  buildWithToolResults(originalMessage, aiResponse, toolResults) {
    ...
    prompt += `Based on the tool results, please provide your final response. Include the actual solution or next steps.`;
    
    return prompt;
  }

  // For compacting conversations
  async compactConversation(conversation, llm) {
    if (conversation.length < 20) return conversation;
    
    const toSummarize = conversation.slice(0, -10);
    const recent = conversation.slice(-10);
    
    const summaryPrompt = `Summarize this conversation, keeping only key decisions, important code changes, and relevant context. Be concise.`;
    
    let summary = '';
    if (llm) {
      summary = await llm.chat([
        { role: 'system', content: summaryPrompt },
        { role: 'user', content: JSON.stringify(toSummarize.map(m => ({ role: m.role, content: m.content.slice(0, 300) }))) }
      ]);
    } else {
      summary = `[Previous conversation with ${toSummarize.length} messages summarized]`;
    }
    
    return [{ role: 'system', content: `Summary: ${summary}` }, ...recent];
  }
}
