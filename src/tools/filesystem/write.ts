  ...
  handler: async (params, context): Promise<ToolResult> => {
    ...
    try {
      ...
      if (existed && createBackup) {
        ...
        const existing = await fs.readFile(absolutePath, encoding as BufferEncoding);
        await fs.writeFile(backupPath, existing, encoding as BufferEncoding);
      }
      ...
      return { success: false, output: '', error: `Write error: ${(error as Error).message}` };
    }
  },
};
