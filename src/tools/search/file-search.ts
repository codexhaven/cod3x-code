    ...
    try {
      ...
      if (params.minSize || params.maxSize || params.modifiedAfter) {
        ...
        for (const f of files.slice(0, 200)) {
          ...
          if (!stats) continue;
          if (params.minSize && stats.size < (params.minSize as number)) continue;
          if (params.maxSize && stats.size > (params.maxSize as number)) continue;
          if (params.modifiedAfter && stats.mtime < new Date(params.modifiedAfter as string)) continue;
