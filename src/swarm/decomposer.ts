...
import { AgentTask, TaskGraph, TaskNode, TaskEdge, Logger, LLMProvider } from '@codex-types/index';
...
export class TaskDecomposer {
   ...
   */
  async decompose(objective: string, context?: string): Promise<TaskGraph> {
...
Be specific and actionable. Each subtask should be independently executable.`;
    ...
    try {
      ...
      this.logger.warn('LLM decomposition failed, using heuristic', { error: (error as Error).message });
   ...
   */
  private parseDecomposition(response: string, originalObjective: string): TaskGraph {
    ...
    while ((match = taskRegex.exec(response)) !== null) {
      ...
      const priority = (block.match(priorityRegex)?.[1] || 'medium').toLowerCase() as any;
   ...
   */
  private heuristicDecompose(objective: string): TaskGraph {
    const nodes: TaskNode[] = [];
    const edges: TaskEdge[] = [];
    ...
    if (lower.includes('create') || lower.includes('build') || lower.includes('implement')) {
      nodes.push(
        { id: 'subtask-1', task: { id: 'subtask-1', description: `Analyze requirements: ${objective}`, priority: 'high', dependencies: [] }, status: 'pending' },
        { id: 'subtask-2', task: { id: 'subtask-2', description: `Design structure for: ${objective}`, priority: 'high', dependencies: ['subtask-1'] }, status: 'pending' },
        { id: 'subtask-3', task: { id: 'subtask-3', description: `Implement core functionality: ${objective}`, priority: 'critical', dependencies: ['subtask-2'] }, status: 'pending' },
        { id: 'subtask-4', task: { id: 'subtask-4', description: `Add error handling and validation`, priority: 'medium', dependencies: ['subtask-3'] }, status: 'pending' },
        { id: 'subtask-5', task: { id: 'subtask-5', description: `Write tests for: ${objective}`, priority: 'medium', dependencies: ['subtask-3'] }, status: 'pending' },
      );

      edges.push(
        { from: 'subtask-1', to: 'subtask-2', type: 'depends_on' },
        { from: 'subtask-2', to: 'subtask-3', type: 'depends_on' },
        { from: 'subtask-3', to: 'subtask-4', type: 'depends_on' },
        { from: 'subtask-3', to: 'subtask-5', type: 'depends_on' },
      );
    } else if (lower.includes('fix') || lower.includes('debug') || lower.includes('error')) {
      nodes.push(
        { id: 'subtask-1', task: { id: 'subtask-1', description: `Investigate the issue: ${objective}`, priority: 'critical', dependencies: [] }, status: 'pending' },
        { id: 'subtask-2', task: { id: 'subtask-2', description: `Identify root cause`, priority: 'critical', dependencies: ['subtask-1'] }, status: 'pending' },
        { id: 'subtask-3', task: { id: 'subtask-3', description: `Implement fix`, priority: 'high', dependencies: ['subtask-2'] }, status: 'pending' },
        { id: 'subtask-4', task: { id: 'subtask-4', description: `Verify fix and add regression test`, priority: 'high', dependencies: ['subtask-3'] }, status: 'pending' },
      );
      ...
      nodes.push(
        { id: 'subtask-1', task: { id: 'subtask-1', description: `Analyze current code: ${objective}`, priority: 'high', dependencies: [] }, status: 'pending' },
        { id: 'subtask-2', task: { id: 'subtask-2', description: `Identify improvement areas`, priority: 'high', dependencies: ['subtask-1'] }, status: 'pending' },
        { id: 'subtask-3', task: { id: 'subtask-3', description: `Apply refactoring changes`, priority: 'critical', dependencies: ['subtask-2'] }, status: 'pending' },
        { id: 'subtask-4', task: { id: 'subtask-4', description: `Run tests to verify`, priority: 'high', dependencies: ['subtask-3'] }, status: 'pending' },
      );
      ...
      nodes.push(
        { id: 'subtask-1', task: { id: 'subtask-1', description: objective, priority: 'high', dependencies: [] }, status: 'pending' },
      );
    }
   ...
   */
  private getAncestors(nodeId: string, graph: TaskGraph): Set<string> {
  ...
  private findParallelGroups(graph: TaskGraph): string[][] {
    ...
    for (const node of graph.nodes) {
      ...
      for (const other of graph.nodes) {
        ...
        const canParallel = graph.edges.some(e => 
          (e.from === node.id && e.to === other.id && e.type === 'parallel_with') ||
